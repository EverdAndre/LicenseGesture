function configurarAutocomplete(config) {
    ...
    const campoId = config.campoId ? document.getElementById(config.campoId) : null;
    const resultado = document.getElementById(config.resultadoId);

    if (!campoBusca || !resultado || (config.campoId && !campoId)) {
        return;
    }
    ...
    const selecionarItem = function (index) {
        ...
        if (config.enviarAoSelecionar) {
            campoBusca.form?.requestSubmit();
        }
    };
    ...
    campoBusca.addEventListener("input", async function () {
        ...
        const fontes = config.fontes ?? [{ url: config.url }];

        try {
            const respostas = await Promise.all(fontes.map(function (fonte) {
                return fetch(`${fonte.url}?busca=${encodeURIComponent(busca)}`, {
                    signal: controleBusca.signal
                });
            }));

            if (respostas.some(function (resposta) { return !resposta.ok; })) {
                throw new Error("Falha ao buscar sugestoes.");
            }

            const resultados = await Promise.all(respostas.map(function (resposta) {
                return resposta.json();
            }));

            itens = resultados.flatMap(function (lista, index) {
                const tipo = fontes[index].tipo;
